/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Matthew Mammano and Andrew Yurovchak
 * Version     : 1.0
 * Date        : 11/20/23
...
public:
     ...
     * vector.
     */
    explicit RedBlackTree(std::vector< std::pair<K, V> > &elements) : root_(nullptr), size_(0) { insert_elements(elements); }
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        // const K& key = key_value.first;  // Matt: I don't think we need this line so I commented out (but it was provided in OG code)
        Node<K, V> *x, *y;
        if (it != end()) {
            x = it.node_ptr;
            y = x->parent;
        ...
        while (x != nullptr) {
            ...
            else throw tree_exception("ALREADY IN THIS, SO DO NOT INSERT IT STUPID");
        }
        z->parent = y;
